...
export const getProcessingMessages = query({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db
      .query("messages")
      .withIndex("by_project_status", (q) => q.eq("projectId", args.projectId).eq("status", "processing"))
      .collect()
  }
})
...
export const renameFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existing = siblings.find(
      (sibling) => sibling.name === args.newName && sibling.type === file.type && sibling._id !== args.fileId
    )
...
export const updateImportStatus = mutation({
  args: {
    internalKey: v.string(),
    projectId: v.id("projects"),
    status: v.optional(v.union(v.literal("importing"), v.literal("completed"), v.literal("failed")))
  },
  handler: async (ctx, args) => {
    validateInternalKey(args.internalKey)
