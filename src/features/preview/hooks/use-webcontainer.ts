...
export const useWebContainer = ({
    ...
    // Reset state when disabled (adjust during render, not in an effect,
    // to avoid the extra cascading render effects would cause)
    ...
    // Reset the boot guard ref when disabled (refs can only be mutated
    // outside of render, so this stays in an effect)
    useEffect(() => {
        if (!enabled) {
